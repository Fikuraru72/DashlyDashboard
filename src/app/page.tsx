...
import { Activity, ArrowRight, ShieldAlert, Zap, MapPin, Calendar, Users, Smartphone, Globe, Mail } from "lucide-react";
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Show all events (upcoming, registered, live, done)
        setEvents(eventList);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <a href="#events" className="text-sm font-medium text-slate-300 hover:text-white transition-colors">Events</a>
            <a href="#contact" className="text-sm font-medium text-slate-300 hover:text-white transition-colors">Contact</a>
          </div>
          <div className="flex items-center gap-4">
            <Link href="/login" className="text-sm font-medium text-slate-300 hover:text-white hidden md:block transition-colors">
              Organizer Login
            </Link>
            <a
              href={downloadUrl}
      ...
      <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-cyan-600/20 rounded-full mix-blend-screen filter blur-[120px] animate-pulse" style={{ animationDelay: '2s' }}></div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <p className="max-w-2xl mx-auto text-lg md:text-xl text-slate-400 mb-10 leading-relaxed">
            Dashly brings real-time GPS tracking, anomaly detection, and instant emergency responses to your fingertips. The ultimate companion for endurance events.
          </p>
      ...
      <section id="about" className="py-24 relative z-10 border-t border-white/5 bg-slate-900/50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-3xl md:text-5xl font-black mb-6">Built for <span className="text-indigo-400">Safety</span> & <span className="text-cyan-400">Performance</span>.</h2>
              <p className="text-slate-400 text-lg leading-relaxed mb-6">
                Endurance racing pushes human limits. When athletes are out on the trail, every second counts. Dashly was built to ensure no runner goes untracked, and no emergency goes unnoticed.
              </p>
      ...
      <section id="features" className="py-24 relative z-10">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-slate-400 text-lg">Everything you need to monitor, manage, and execute flawless racing events.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: MapPin, title: "Live Tracking", desc: "Track every participant on a unified geographic map with sub-second latency.", color: "text-indigo-400", bg: "bg-indigo-500/10" },
              { icon: ShieldAlert, title: "Anomaly Detection", desc: "Automated alerts for stationary participants or route deviations.", color: "text-rose-400", bg: "bg-rose-500/10" },
              { icon: Smartphone, title: "Mobile Ready", desc: "Participant companion app for registration, QR tickets, and live tracking.", color: "text-cyan-400", bg: "bg-cyan-500/10" },
              { icon: Zap, title: "Offline Syncing", desc: "GPS coordinates are cached locally and synced instantly upon reconnection.", color: "text-amber-400", bg: "bg-amber-500/10" },
              { icon: Users, title: "Dynamic Quotas", desc: "Real-time registration caps and automatic enrollment closure.", color: "text-emerald-400", bg: "bg-emerald-500/10" },
              { icon: Activity, title: "Dashboard Command", desc: "A central hub for race organizers to monitor health and progress.", color: "text-purple-400", bg: "bg-purple-500/10" }
            ].map((feature, i) => (
              <div key={i} className="p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-colors hover:-translate-y-1 duration-300">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            <div>
              ...
              <p className="text-slate-400 text-lg max-w-2xl">Find your next challenge. Register directly through the Dashly app.</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center p-12 bg-white/5 border border-white/10 rounded-3xl">
              <p className="text-slate-400">No public events available right now. Check back later!</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-3 gap-8">
              {events.map((event) => (
                ...
                >
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-bold mb-2 group-hover:text-indigo-400 transition-colors line-clamp-1">{event.name}</h3>
                    <p className="text-slate-400 text-sm mb-4 line-clamp-2">{event.description || "No description provided."}</p>
                    <div className="space-y-2 mb-6">
                      <div className="flex items-center gap-2 text-sm text-slate-300">
                        <Calendar className="w-4 h-4 text-indigo-400" />
                        <span>{event.dateEvent ? new Date(event.dateEvent).toLocaleDateString() : 'TBA'}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-slate-300">
                      ...
                      <div className="flex items-center gap-2 text-sm text-slate-300">
                        ...
                        <span>{event.currentCount} / {event.maxParticipants} Registered</span>
                      </div>
                    </div>
