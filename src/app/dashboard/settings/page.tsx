...
export default function SettingsPage() {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await authenticatedFetch(`${apiUrl}/users/me`, {
        method: "PATCH",
        headers: {
      ...
      void fetchUser(); // Refresh global user state
  ...
  if (!user) {
    return (
      <div className="flex-1 flex items-center justify-center p-8 h-full bg-slate-50 dark:bg-[#0f172a]">
        <div className="text-slate-500 font-medium animate-pulse tracking-widest uppercase text-sm">
          Loading Settings...
        </div>
      </div>
    );
