...
export default function EventsPage() {
  const {
    ...
    queryKey: ["events"],
    queryFn: async () => {
      const data = await apiFetch<{ success?: boolean; data?: EventSummary[] } | EventSummary[]>(
        `${API_URL}/events`,
      );
      ...
      <div className="mb-8 max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">
            Manage and monitor all racing events across the platform.
          </p>
        </div>
              ...
              >
                ...
                <div className="flex justify-between items-start mb-6">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 uppercase tracking-tight leading-snug pr-2">
                      {evt.name}
                    </h3>
                  </div>

                  <div
                    className={`flex-shrink-0 px-2.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider border flex items-center gap-1.5
                  ${
                    evt.status === "LIVE"
                      ...
                      : evt.status === "IDLE" ||
                        ...
                        : evt.status === "DRAFT" || evt.status === "REGISTRATION_CLOSED"
                ...
                <div className="flex items-center justify-between mb-8 text-sm text-slate-600 dark:text-slate-400 font-medium bg-slate-50/50 dark:bg-slate-950/30 p-4 rounded-xl border border-slate-100 dark:border-slate-800/60">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-slate-800 dark:text-slate-200 text-lg font-black">
                      {evt.currentCount}{" "}
                      <span className="text-xs text-slate-500">/ {evt.maxParticipants}</span>
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-1 text-right">
                    ...
                    <span className="text-indigo-600 dark:text-cyan-400 font-mono text-sm tracking-tight">
                      {evt.id}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-auto flex items-center gap-3">
                  ...
                  <Link
                    ...
                    ${evt.status === "LIVE" ? "flex-[0.5] border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300" : "flex-1 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800/80 shadow-sm hover:scale-[1.02] hover:bg-slate-50 dark:hover:bg-slate-800"}
                  `}
                  >
                    <Eye className="w-4 h-4" />
                    {evt.status === "LIVE" ? "Details" : "Manage Event"}
                  </Link>
                </div>
              </div>
            ))}
