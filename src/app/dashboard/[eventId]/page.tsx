...
export default function DashboardPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const { participants, anomalies, eventMetadata, setParticipants, setEventMetadata, addAnomaly, setSelectedParticipantId, selectedParticipantId } = useParticipantStore();
  const { isConnected } = useSocket(eventId);
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        ...
        const positionsRes = await fetch(`${backendUrl}/events/${eventId}/live`, {
          headers: {
            "Authorization": `Bearer ${token}`
          }
        });
        ...
        if (positionsRes.ok) {
          ...
          console.log("[INIT] ✅ setParticipants executed with map size:", Object.keys(participantMap).length);
        } else {
  ...
  return (
    <div className="flex flex-1 overflow-hidden font-sans text-slate-900 dark:text-slate-50">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">{p.status === 'stuck' ? 'Idle' : 'Moving'}</span>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end">
                  <span className="font-bold text-sm text-indigo-600 dark:text-emerald-400">
                    {p.speed.toFixed(1)} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">km/h</span>
                  </span>
                  <div className="flex items-center gap-1 mt-0.5">
                    <Zap className={`h-3 w-3 ${p.battery < 20 ? 'text-rose-500' : 'text-amber-500'}`} />
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">{p.battery}%</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <main className={`flex-1 relative flex items-center justify-center overflow-hidden shadow-inner transition-colors duration-300 ${isFlashing ? 'bg-rose-500/20' : 'bg-slate-200/50 dark:bg-[#0f172a]'}`}>
          ...
          {/* Red Flash Overlay */}
          <div className={`absolute inset-0 pointer-events-none transition-opacity duration-300 z-30 ${isFlashing ? 'opacity-100 bg-rose-500/20' : 'opacity-0'}`}></div>
        </main>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {anomalies.map((alert) => (
              ...
              >
                ...
                <div className={`absolute top-0 left-0 w-1 h-full ${alert.type === 'stuck' ? 'bg-amber-500 dark:bg-amber-400' : 'bg-rose-500 dark:bg-rose-400'
                  }`}></div>

                <div className="flex justify-between items-start mb-2 pl-2">
                  <span className={`text-xs font-bold uppercase tracking-wider ${alert.type === 'stuck' ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'
                    }`}>
                    {alert.type.replace('-', ' ')}
                  </span>
