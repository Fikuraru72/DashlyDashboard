  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/public-events/${eventId}`);
        ...
        try {
          const partsRes = await fetch(`${apiUrl}/public-events/${eventId}/participants`);
          if (partsRes.ok) {
        ...
        try {
          const posRes = await fetch(`${apiUrl}/public-events/${eventId}/live`);
        ...
        try {
          const pathRes = await fetch(`${apiUrl}/public-events/${eventId}/path-history`);
          if (pathRes.ok) {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      if (coords && coords.length > 0) {
        ...
        // Add Minimalist START and FINISH Micro Pills
        if (coords.length > 1) {
          ...
          finishMarkerRef.current = new maplibregl.Marker({ element: finishEl, anchor: "center" })
            .setLngLat([finishCoord[0], finishCoord[1]])
            .addTo(map);
        }
