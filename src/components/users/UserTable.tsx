...
import { Search, Filter, MoreVertical, Eye, Edit2, Trash2, ChevronLeft, ChevronRight, User as UserIcon } from 'lucide-react';
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-50/80 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                            <tr>
                                <th className="p-4 pl-6 font-semibold text-[11px] uppercase tracking-wider text-slate-500 dark:text-slate-400">User</th>
                                <th className="p-4 font-semibold text-[11px] uppercase tracking-wider text-slate-500 dark:text-slate-400 hidden sm:table-cell">Contact</th>
                                <th className="p-4 font-semibold text-[11px] uppercase tracking-wider text-slate-500 dark:text-slate-400">Role</th>
                                <th className="p-4 pr-6 font-semibold text-[11px] uppercase tracking-wider text-slate-500 dark:text-slate-400 text-right">Actions</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={u.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors group">
                                    <td className="p-4 pl-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="font-semibold text-sm text-slate-800 dark:text-slate-200">{u.name}</span>
                                                <span className="text-xs font-medium text-slate-400 sm:hidden">{u.email}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-4 hidden sm:table-cell">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{u.email}</span>
                                    ...
                                    <td className="p-4">
                                        <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold border
                                            ${u.role?.name === 'SUPER_ADMIN' ? 'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-500/10 dark:border-rose-500/20 dark:text-rose-400' : 
                                              u.role?.name === 'STAFF' ? 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-500/10 dark:border-amber-500/20 dark:text-amber-400' :
                                                ...
                                                <>
                                                    ...
                                                    <div className="absolute right-8 top-10 mt-1 w-40 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 py-1 z-20 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                                        ...
                                                        {canManageUsers && (
                                                            <>
                                                                <button 
                                                                    onClick={() => { setActiveDropdownId(null); onEdit(u); }}
                                                                    className="w-full text-left px-4 py-2.5 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 transition-colors"
                                                                >
                                                                ...
                                                                <button 
                                                                    onClick={() => { setActiveDropdownId(null); onDelete(u.id); }}
