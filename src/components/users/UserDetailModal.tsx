        ...
        <>
            ...
            <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 pointer-events-none ${isOpen ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}>
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800/60 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">User Profile</h2>
                    <button 
                        onClick={onClose}
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 space-y-4 shadow-sm">
                            ...
                            <div className="flex items-center gap-3 text-sm">
                                ...
                                <span className="text-slate-700 dark:text-slate-300 font-medium truncate">{user.email}</span>
                            </div>
                            <div className="flex items-center gap-3 text-sm">
                                <div className="w-8 h-8 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center border border-slate-200 dark:border-slate-700 shrink-0">
                                    <Phone className="w-4 h-4 text-slate-500" />
                                </div>
                                <span className="text-slate-700 dark:text-slate-300 font-medium">{user.phone || 'Not provided'}</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center gap-2">
                                ...
                                <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">Medical & Health Record</h4>
                            </div>
                            ...
                            {user.healthInfo ? (
                                <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm relative">
                                    ...
                                    <div className="relative p-5 space-y-4">
                                        {Object.entries(user.healthInfo).map(([key, value]) => (
                                            <div key={key} className="flex flex-col border-b border-slate-100 dark:border-slate-800/60 pb-3 last:border-0 last:pb-0">
                                                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{key.replace(/_/g, ' ')}</span>
                                ...
                                <div className="flex flex-col items-center justify-center p-8 border border-dashed border-slate-200 dark:border-slate-700 rounded-2xl bg-slate-50/50 dark:bg-slate-800/20 text-center">
                                    ...
                                    <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">No health information recorded</p>
                                </div>
                            )}
                        </div>
