...
export default function LocationSearch({ onLocationSelect }: LocationSearchProps) {
  ...
  const handleSelectResult = (feature: any) => {
    ...
    const parts = feature.display_name
      ? feature.display_name.split(",").map((p: string) => p.trim())
      : [];
    ...
    if (feature.address) {
      city =
        ...
        feature.address.municipality ||
        feature.address.county ||
        feature.address.city_district ||
        feature.address.village ||
        city;
      province =
        feature.address.state || feature.address.region || feature.address.province || province;
    }
      ...
      async (position) => {
        ...
        try {
          ...
          if (data && data.address) {
            ...
            const name =
              addr.amenity || addr.building || addr.road || addr.suburb || "Current Location";
            const city =
              addr.city ||
              addr.town ||
              addr.municipality ||
