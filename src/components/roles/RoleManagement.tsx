...
export function RoleManagement({ roles, availablePermissions, onUpdateRole, onCreateRole }: RoleManagementProps) {
            ...
            <div className="lg:col-span-4 space-y-4">
                ...
                <div className="flex flex-col gap-2">
                    {roles.map(role => (
                        ...
                        >
                            <span className={`font-bold text-sm ${activeRoleId === role.id ? 'text-indigo-700 dark:text-cyan-400' : 'text-slate-700 dark:text-slate-200'}`}>
                                {role.name}
                            </span>
            ...
            <div className="lg:col-span-8">
                {activeRole ? (
                        ...
                        <div className="p-6 border-b border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            ...
                            >
                                {successId === activeRole.id ? (
                                    ...
                                    <><Save className="w-4 h-4" /> {savingId === activeRole.id ? 'Saving...' : 'Save Changes'}</>
                                )}
                            </button>
                        </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex flex-col">
                                            <span className={`font-bold text-sm transition-colors ${isChecked ? 'text-indigo-700 dark:text-cyan-400' : 'text-slate-700 dark:text-slate-300'}`}>
                    ...
                    <div className="h-full min-h-[400px] flex flex-col items-center justify-center p-8 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-white/50 dark:bg-slate-900/50">
                        ...
                        <p className="text-sm font-medium text-slate-400 dark:text-slate-500 mt-1">Choose a role from the sidebar to view and edit its permissions.</p>
                    </div>
                )}
            </div>
