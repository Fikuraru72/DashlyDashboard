  ...
  const navItems = [
    {
      ...
      icon: CalendarDays,
      active: pathname === "/dashboard/events" || pathname === "/dashboard",
    },
        ...
        <nav className="flex-1 py-6 flex flex-col gap-2 px-3 overflow-y-auto overflow-x-hidden">
          ...
          {isMonitoringContext && activeEventId && (
            ...
            >
              ...
              <Activity
                className={`h-5 w-5 flex-shrink-0 ${isMonitoringContext ? "animate-pulse" : ""}`}
              />
              <span
                className={`font-bold text-sm whitespace-nowrap transition-all duration-300 ${isCollapsed ? "opacity-0 translate-x-4 w-0 hidden" : "opacity-100 translate-x-0"}`}
              >
                Live Monitoring
              </span>
            </Link>
          ...
          {navItems.map((item) => (
            ...
            >
              ...
              <span
                className={`font-semibold text-sm whitespace-nowrap transition-all duration-300 ${isCollapsed ? "opacity-0 translate-x-4 w-0 hidden" : "opacity-100 translate-x-0"}`}
              >
                {item.name}
              </span>
            </Link>
