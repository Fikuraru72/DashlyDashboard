...
export function Header() {
  ...
  const isMonitoring = segments.length === 2 && segments[0] === "dashboard" && !["events", "users", "history", "settings"].includes(segments[1]);
            ...
            <div className="flex items-center gap-2">
              ...
              {eventMetadata.dateEvent && (
                <>
                  ...
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest hidden sm:block">
                    {new Date(eventMetadata.dateEvent).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
                  </p>
                </>
              )}
            </div>
          ...
          <>
            <div className="hidden lg:flex items-center gap-4 mr-2">
              <div className="flex flex-col items-end">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Participants</span>
                <span className="text-lg font-bold leading-none text-slate-800 dark:text-slate-200">{totalParticipantsCount}</span>
              </div>
              <div className="h-6 w-px bg-slate-200 dark:bg-slate-700"></div>
              <div className="flex flex-col items-end">
                <span className="text-[10px] font-bold uppercase tracking-wider text-rose-500">Alerts</span>
                <span className="text-lg font-bold leading-none text-rose-600 dark:text-rose-400">{activeAnomaliesCount}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-full text-sm font-medium border border-slate-200 dark:border-slate-700 shadow-sm">
              <span className={`w-2 h-2 rounded-full shadow-lg ${socketConnected ? "bg-emerald-500 animate-pulse shadow-emerald-500/50" : "bg-rose-500 shadow-rose-500/50"}`}></span>
